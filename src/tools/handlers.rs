use super::helpers::{
    do_multi_search, do_search, format_results, resolve_engines, results_to_text,
};
...
use crate::{
    ...
    utils::url_safety::{is_public_http_url, is_url_from_host},
};
use rmcp::{
    ServerHandler,
...
impl WebSearchHandler {
    ...
    pub async fn web_search(&self, p: Parameters<WebSearchParams>) -> String {
        ...
        let raw: Vec<String> = params.engines.unwrap_or_default();
        let engines = resolve_engines(&raw);
        ...
        let multi = do_multi_search(&engines, &params.query, limit).await;

        if multi.results.is_empty() {
            if multi.errors.is_empty() {
                return "No results found.".into();
            }
            let err_detail: Vec<String> = multi
                .errors
                .iter()
                .map(|(e, msg)| format!("{}: {}", e, msg))
                .collect();
            return format!("All engines failed:\n{}", err_detail.join("\n"));
        }
        ...
        if !multi.errors.is_empty() {
            output.push_str("\n## Warnings\n\n");
            for (engine, msg) in &multi.errors {
                output.push_str(&format!("- Engine '{}' failed: {}\n", engine, msg));
            }
        }
    ...
    pub async fn fetch_github_readme_tool(&self, p: Parameters<GithubReadmeParams>) -> String {
        let url = p.0.url;
        if !is_url_from_host(&url, "github.com") {
    ...
    pub async fn fetch_csdn_article_tool(&self, p: Parameters<ArticleUrlParams>) -> String {
        let url = p.0.url;
        if !is_url_from_host(&url, "csdn.net") {
    ...
    pub async fn fetch_juejin_article_tool(&self, p: Parameters<ArticleUrlParams>) -> String {
        let url = p.0.url;
        if !is_url_from_host(&url, "juejin.cn") || !url.contains("/post/") {
    ...
    pub async fn fetch_zhihu_article_tool(&self, p: Parameters<ArticleUrlParams>) -> String {
        let url = p.0.url;
        if !is_url_from_host(&url, "zhihu.com") {
    ...
    pub async fn fetch_linuxdo_article_tool(&self, p: Parameters<ArticleUrlParams>) -> String {
        let url = p.0.url;
        if !is_url_from_host(&url, "linux.do") || !url.contains("/topic/") {
